...
### 2.2 通过 API Server 获取 Pod 清单及创建 Pod 的过程
...
cAdvisor 通过其所在节点机的 4194 端口暴露一个简单的 UI。（确认kubelet中有cAdvisor部分，但没有发现有监听4194端口）

## 5. Kubelet Eviction（驱逐）
...
- Kubelet API，包括 10250 端口的认证 API、10255 端口的只读 API 以及 10248 端口的健康检查 API
