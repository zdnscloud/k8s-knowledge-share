...
```
> 请注意，定义了3个不同的概率，而不是到处都定义为0.33。原因是规则是顺序执行的。

> 第一个规则的执行概率为0.33，执行概率为33%，跳过概率为66%

> 第二个规则的执行概率为0.5，执行的概率为50%，跳过的概率为50%。但是，由于这个规则放在第一个规则之后，所以它的执行概率只有66%。因此，此规则将仅应用于(50/%*66/%=33/%)的请求

> 由于只有33%的流量达到了最后一条规则，所以必须始终应用它
